...
test("scenario: long-running disconnected poll expiry preserves running state until resync", () => {
  const cellStatus = "running";
  const expiry = resolvePollWindowExpiry({
    currentStatus: cellStatus,
    wsConnected: false,
  });

  assert.equal(expiry.shouldUpdateStatus, false);
  assert.equal(expiry.shouldInjectSyntheticError, false);
  assert.equal(expiry.nextStatus, "running");
  assert.equal(expiry.logLevel, "warn");
});
